...
import alk.asm.loader.interfaces.IClassNameTransformer;
...
public class AsmClassLoader extends URLClassLoader {
    ...
    public AsmClassLoader(URL[] sources) {
        ...
        addClassLoaderExclusion("jakarta.servlet.Servlet");
        ...
        if (DEBUG_SAVE) {
            int x = 1;
            tempFolder = new File(Main.sHomeDir, "CLASSLOADER_TEMP");
            while (tempFolder.exists() && x <= 10) {
                tempFolder = new File(Main.sHomeDir, "CLASSLOADER_TEMP" + x++);
            }

            if (tempFolder.exists()) {
                Log.info("DEBUG_SAVE enabled, but 10 temp directories already exist, clean them and try again.");
                tempFolder = null;
            } else {
                Log.info("DEBUG_SAVE Enabled, saving all classes to \"%s\"", tempFolder.getAbsolutePath().replace('\\', '/'));
                tempFolder.mkdirs();
            }
        }
    }
    ...
    public void registerTransformer(String transformerClassName) {
        try {
            ...
            Log.log(Level.ERROR, e, "A critical problem occurred registering the ASM transformer class %s", transformerClassName);
        }
    }

    @Override
    public Class<?> findClass(final String name) throws ClassNotFoundException {
    	//Log.log("Trying to find Class: "+name);
        ...
        for (final String exception : classLoaderExceptions) {
            if (name.startsWith(exception)) {
            	//Log.log("Found Class Loader Exclusion, using parent.loadClass() "+name);
                return parent.loadClass(name);
            }
        }

        if (cachedClasses.containsKey(name)) {
        	Log.log("Found cached Class. "+name);
        ...
        for (final String exception : transformerExceptions) {
            if (name.startsWith(exception)) {
                try {
                	//Log.log("Found Transformer Exclusion, using super.loadClass()");
        ...
        try {
        	Log.log("Trying to do stuff. "+name);
            ...
            if (lastDot > -1 && !untransformedName.startsWith("net.minecraft.")) {
                if (urlConnection instanceof JarURLConnection) {
                    ...
                    if (jarFile != null && jarFile.getManifest() != null) {
                        ...
                        if (pkg == null) {
                            ...
                            if (pkg.isSealed() && !pkg.isSealed(jarURLConnection.getJarFileURL())) {
                                Log.severe("The jar file %s is trying to seal already secured path %s", jarFile.getName(), packageName);
                            } else if (isSealed(packageName, manifest)) {
                                Log.severe("The jar file %s has a security seal for path %s, but that path is defined and not secure", jarFile.getName(), packageName);
                            }
                        }
                    }
                } else {
                    ...
                    if (pkg == null) {
                        ...
                        Log.severe("The URL %s is defining elements for sealed path %s", urlConnection.getURL(), packageName);
                    }
                }
            }
        	...
        	Log.log("Failed to do stuff.");
        	e.printStackTrace();
            invalidClasses.add(name);
            if (DEBUG) {
                Log.log(Level.TRACE, e, "Exception encountered attempting classloading of %s", name);
    ...
    private void saveTransformedClass(final byte[] data, final String transformedName) {
        ...
        try {
            Log.fine("Saving transformed class \"%s\" to \"%s\"", transformedName, outFile.getAbsolutePath().replace('\\', '/'));

            final OutputStream output = new FileOutputStream(outFile);
            output.write(data);
            output.close();
        } catch (IOException ex) {
            Log.log(Level.WARN, ex, "Could not save transformed class \"%s\"", transformedName);
        }
    }
    ...
    private byte[] runTransformers(final String name, final String transformedName, byte[] basicClass) {
        if (DEBUG_FINER) {
            Log.finest("Beginning transform of {%s (%s)} Start Length: %d", name, transformedName, (basicClass == null ? 0 : basicClass.length));
            for (final IClassTransformer transformer : transformers) {
                final String transName = transformer.getClass().getName();
                Log.finest("Before Transformer {%s (%s)} %s: %d", name, transformedName, transName, (basicClass == null ? 0 : basicClass.length));
                basicClass = transformer.transform(name, transformedName, basicClass);
                Log.finest("After  Transformer {%s (%s)} %s: %d", name, transformedName, transName, (basicClass == null ? 0 : basicClass.length));
            }
            Log.finest("Ending transform of {%s (%s)} Start Length: %d", name, transformedName, (basicClass == null ? 0 : basicClass.length));
        } else {
            for (final IClassTransformer transformer : transformers) {
                basicClass = transformer.transform(name, transformedName, basicClass);
            }
    ...
    private byte[] readFully(InputStream stream) {
        try {
            ...
            Log.log(Level.WARN, t, "Problem loading class");
            return new byte[0];
        }
    }
    ...
    public byte[] getClassBytes(String name) throws IOException {
        ...
        try {
            ...
            if (classResource == null) {
                if (DEBUG) Log.finest("Failed to find class resource %s", resourcePath);
                negativeResourceCache.add(name);
                return null;
            }
            classStream = classResource.openStream();

            if (DEBUG) Log.finest("Loading class %s from resource %s", name, classResource.toString());
